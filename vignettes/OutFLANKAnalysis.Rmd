...
Before doing running the OutFLANK() function to estimate the parameters on the neutral FST distribution, you will want to identify a quasi-random set of SNPs. We have provided the indexes of this quasi-random set of SNPs with the data distributed in the package:
...
Our pruned SNP set is a couple thousand SNPs fewer than our full dataset. For information on how we obtained these SNPs, see the "Bonus" section at the end of the vignette.
...
DISCLAIMER: Note that this dataset does not include missing data, so this code may not work in all scenarios. Also, NA should be replaced with "9" to work with the functions in the package.

## Bonus: code used to obtain pruned SNPs 
Here is the code that we used to obtain the pruned SNPs. Note that this code is not run with the vignette. Note also that your chromosome needs to be of class `integer` for this to work. See the `bigsnpr` package for more information.
